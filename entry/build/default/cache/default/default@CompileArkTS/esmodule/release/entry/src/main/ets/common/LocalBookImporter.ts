...
import { BookParserInfo, LocalBookImportResult } from "@bundle:liubai.yuedu.hos/entry/ets/common/BookParserInfo";
import { BookUtils } from "@bundle:liubai.yuedu.hos/entry/ets/utils/BookUtils";
import { FileUtils } from "@bundle:liubai.yuedu.hos/entry/ets/utils/FileUtils";
...
export class LocalBookImporter {
    ...
    private async checkFileValid(filePath: string): Promise<LocalBookImportResult> {
        ...
        try {
            ...
            hilog.error(0x0000, TAG, "checkFileValid LOCAL_BOOK_IMPORT_FILE_SYSTEM_ERROR.message: ", err.message, ", error code: ", err.code);
    ...
    private async handleLocalBookFileCache(filePath: string, workPath: string, bookParserInfo: BookParserInfo): Promise<BookParserInfo> {
        try {
    ...
    private async copyBookToCache(filePath: string, dstPath: string, bookParserInfo: BookParserInfo): Promise<void> {
        try {
            ...
            hilog.error(0x0000, TAG, "copyBookToCache LOCAL_BOOK_IMPORT_FILE_SYSTEM_ERROR : ", err.message, ", error code: ", err.code);
    ...
    private async getLocalBookBasicParserInfo(filePath: string): Promise<BookParserInfo> {
        try {
            ...
            hilog.error(0x0000, TAG, "getLocalBookBasicParserInfo LOCAL_BOOK_IMPORT_FILE_SYSTEM_ERROR : ", err.message, ", error code: ", err.code);
            return Promise.reject(LocalBookImportResult.LOCAL_BOOK_IMPORT_FILE_SYSTEM_ERROR);
        }
    }
}
